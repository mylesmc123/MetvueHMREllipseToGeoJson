...
Because I havent figured out how to correctly project the .tin data in Geopandas, some manual processing is involved:
    Input .tin file to TINtoPts.ipynb and outputs two files:
        1. .csv with the wrong scale but correct center
        2. .gpkg with the correct scale but wrong center (For example, instead of Friendswood, the center is over by Austin.)
...
The output geojson, ./out.json, is now ready to be added to FloodMap.